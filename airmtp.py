...
DEFAULT_MAX_KB_TO_BUFFER_FOR_GET_OBJECT_REQUESTS = (
    32768  # 32MB - max bytes we buffer before flushing what we have to disk
)
...
ERRNO_CAMERA_UNEXPECTED_RESP_CODE = (
    4001  # an MTP request was successfully delivered but the camera responsed with response code that indicated failure
)
...
ERRNO_FIRST_CUSTOM_DONT_RETRY = (
    5000  # first custom errno that we trigger for which app should bypass any retry invocations
)
ERRNO_BAD_CMD_LINE_ARG = 5000  # bad command line argument specified
ERRNO_FILE_EXISTS_USER_SPECIFIED_EXIT = (
    5001  # a file exists that we were to write to and user configured app to exit when this occurs
)
ERRNO_DIFFERENT_CAMREA_DURING_RETRY = (
    5002  # during a retry  invocation a different camera was discvered vs the original camera we found
)
ERRNO_NO_CAMERA_TRANSFER_LIST = (
    5003  # no camera transfer list available and user configured app to exit if the list is not available
)
...
ERRNO_TRANSFER_LIST_NOT_SUPPORTED = (
    5009  # camera doesn't support MTP transfer lists, which is used to download user-selected images in the camera
)
ERRNO_FILENAMESPEC_RESULT_EMPTY_STR = (
    5010  # the result of the user-specified --filenamespec resulted in an empty string
)
ERRNO_FILENAMESPEC_HAS_PATH_CHARACTERS = (
    5011  # the result of the user-specified --filenamespec had a path or path characters in it
)
ERRNO_DOWNLOADEXEC_LAUNCH_ERROR = 5012  # unable to launch app/script specified by --downloadexec
ERRNO_DOWNLOADEXEC_NON_ZERO_EXIT_CODE = (
    5013  # a launched '--downloadexec' app returned a non-zero result and user config was to exit on this case
)
ERRNO_LAST_CUSTOM_DONT_RETRY = (
    5099  # last custom errno that we trigger for which app should bypass any retry invocations
)
...
class DownloadStatsStruct:
    ...
    def reportDownloadStats(self, fDontPrintStatsIfNoFilesDownloaded=False):
        ...
        if fDontPrintStatsIfNoFilesDownloaded and g.dlstats.countFilesDownloaded == 0:
            return
        applog_i(
            "\n{:d} files downloaded in {:.2f} seconds (Average Rate = {:.2f} MB/s)".format(
                g.dlstats.countFilesDownloaded, g.dlstats.totalDownloadTimeSecs, averageDownloadRateMbSec
            )
        )
        ...
        if g.dlstats.countFilesSkippedDueToFileExistingLocally:
            applog_i(
                "{:d} files skipped because they already existed in output directory".format(
                    g.dlstats.countFilesSkippedDueToFileExistingLocally
                )
            )
...
class GlobalVarsStruct:
    def __init__(self):
        ...
        self.objfilter_dateStartEpoch = (
            None  # user-specified starting date filter. any file earlier than this will be filtered.
        )
        self.objfilter_dateEndEpoch = (
            None  # user-specified ending date filter. any file later than this will be filtered.
        )
        ...
        self.cameraLocalMetadataPathAndRootName = (
            None  # path+root name for all metadata files we associate with a specific model+serial number
        )
        ...
        self.downloadMtpFileObjects_LastMtpObjectDownload = (
            None  # MTP object last downloaded (either last completed or last we were working on)
        )

        self.countFilesDownloadedPersistentAcrossStatsReset = (
            0  # count of files downloaded this session, survives reset of DownloadStatsStruct
        )
...
class MtpObject(LinkedListObj):
    ...
    # class variables
    #
    __MtpObjects_LL_CaptureDateSorted = (
        LinkedList()
    )  # link list of all MtpObject instances, sorted by capture date ([0] = oldest, [n-1]=newest)
    ...
    def __init__(self, mtpObjectHandle, mtpObjectInfo):
        ...
        if self.mtpObjectInfo.captureDateStr:  # there is a non-empty capture date string
            ...
            if self.mtpObjectInfo.associationType == MTP_OBJASSOC_GenericFolder:
                if (
                    len(self.mtpObjectInfo.filename) == 10
                    and self.mtpObjectInfo.filename[4] == "-"
                    and self.mtpObjectInfo.filename[7] == "-"
                ):
        ...
        if MtpObject.objInList(self):
            raise AssertionError(
                "MtpObject: Attempting to insert mtpObjectHandle that's already in dictionary. newObj:\n{:s}, existingObj:\n{:s}".format(
    ...
    def getImmediateDirectory(self):  # gets immediate camera directory that this object is in. ex: "100NC1J4"
        ...
        if objHandleDirectory not in MtpObject.__MtpObjects_ObjectHandleDict:
            applog_d(
                "getImmediateDirectory(): Unable to locate parent object for {:s}, parent=0x{:08x}".format(
                    self.mtpObjectInfo.filename, objHandleDirectory
                )
            )
            return ""
    ...
    def genFullPathStr(
        self,
    ):  # builds full path string to this object on camera, including filename itself. Ex: "DCIM\100NC1J4\DSC_2266.NEF"
        ...
        while objHandleAncestorDirectory != 0:
            if objHandleAncestorDirectory not in MtpObject.__MtpObjects_ObjectHandleDict:
                ...
                applog_d(
                    "genFullPathStr(): Unable to locate parent object for {:s}, parent=0x{:08x}".format(
                        self.mtpObjectInfo.filename, objHandleDirectory
                    )
                )
                return pathStr
            ...
            if loopIterationCounter_EndlessLoopProtectionFromCorruptList >= 512:
                ...
                raise AssertionError(
                    "Endless loop detected while building directory chain for {:s}. Local list is corrupt".format(
                        pathStr
                    )
                )
    ...
    def __str__(self):  # generates string description of object
        ...
        s += (
            "    thumbPixDimensions = "
            + str(self.mtpObjectInfo.thumbPixWidth)
            + "x"
            + str(self.mtpObjectInfo.thumbPixHeight)
            + "\n"
        )
        s += (
            "    imagePixDimensions = "
            + str(self.mtpObjectInfo.imagePixWidth)
            + "x"
            + str(self.mtpObjectInfo.imagePixHeight)
            + "\n"
        )
...
def verifyPythonVersion():
    if sys.version_info.major == 2:
        if sys.version_info.minor < 7:
            ...
            applog_i(
                "Version running is {:d}.{:d}.{:d}, app was tested on 2.7.x".format(
        ...
        if sys.version_info.minor < 4:
            ...
            applog_i(
                "Version running is {:d}.{:d}.{:d}, app was tested on 3.4.x".format(
                    sys.version_info.major, sys.version_info.minor, sys.version_info.micro
                )
            )
...
def establishAppEnvironment():
    ...
    g.isFrozen = getattr(
        sys, "frozen", False
    )  # note for OSX isFrozen is always false because py2app only marks airnef.pyw as frozen when we're a py2app
    ...
    if g.isFrozen and g.isWin32:
        if os.getenv("LOCALAPPDATA"):
            g.appDataDir = os.path.join(
                os.getenv("LOCALAPPDATA"), "airmtp\\appdata"
            )  # typically C:\Users\<username>\AppData\Local\airnef\appdata
...
def translateDateCmdLineArgToEpoch(cmdArgDesc, isInclusiveEndDate=False):
    ...
    try:
        ...
        applog_e(
            'Date specified for "--{:s}" is "{:s}", which is formatted incorrectly or has an invalid date/time. It must be formatted as mm/dd/yy or mm/dd/yy hh:mm:ss (including leading zeros) and be a valid date/time.'.format(
                cmdArgDesc, userDateTimeStr
            )
        )
    ...
    if bOnlyDateSpecified and isInclusiveEndDate:
        timeEpoch += (
            (23 * 60 * 60) + (59 * 60) + 59
        )  # make end date inclusive by adding 23 hours, 59 minutes, 59 seconds to epoch time
    return timeEpoch
...
def verifyIntegerArgRange(argNameStr, minAllowedValue, maxAllowedValue):
    if g.args[argNameStr] != None:
        if g.args[argNameStr] < minAllowedValue or g.args[argNameStr] > maxAllowedValue:
            applog_e(
                "Invalid value for --{:s}: valid range is [{:d}-{:d}]".format(
                    argNameStr, minAllowedValue, maxAllowedValue
                )
            )
...
class ArgumentParserError(Exception):
    pass  # from http://stackoverflow.com/questions/14728376/i-want-python-argparse-to-throw-an-exception-rather-than-usage
...
def processCmdLine():
    ...
    #
    parser = ArgumentParserWithException(
    ...
    parser.add_argument(
        "--ipaddress",
        type=str.lower,
        help='IP address of camera. Default is "%(default)s"',
        default="192.168.1.1",
        metavar="addr",
        required=False,
    )
    parser.add_argument(
        "--action",
        type=str.lower,
        choices=["getfiles", "getsmallthumbs", "getlargethumbs", "listfiles"],
        help='Program action. Default is "%(default)s"',
        default="getfiles",
        required=False,
    )
    parser.add_argument(
        "--realtimedownload",
        type=str.lower,
        choices=["disabled", "afternormal", "only"],
        help="Download images from camera in realtime as they're taken. 'afternormal' means realtime capture starts after regular image download. 'only' skips normal download and only captures realtime images. Default is \"%(default)s\"",
        default="disabled",
        required=False,
    )
    parser.add_argument(
        "--extlist",
        help='Type of image/file(s) to download. Ex: "--extlist NEF". Multiple extensions can be specified. Use "<NOEXT>" to include files that don\'t have extensions. Default is to download all file types',
        default=None,
        nargs="+",
        metavar="extension",
        required=False,
    ...
    parser.add_argument(
        "--startdate",
        help='Only include image/file(s) captured on or later than date. Date-only Ex: --startdate 12/05/14. Date+Time Example: --startdate "12/05/14 15:30:00"',
        metavar="date",
        required=False,
    )
    parser.add_argument(
        "--enddate",
        help="Only include image/file(s) captured on or earlier than date or date+time. Date without a specified time is inclusive, so for example --enddate 06/12/14 is interpreted as 06/12/14 23:59:59",
        metavar="date",
        required=False,
    )
    parser.add_argument(
        "--outputdir",
        type=str,
        help='Directory to store image/file(s) to.  Default is current directory. No ending backslash is necessary. If path contains any spaces enclose it in double quotes. Example: --outputdir "c:\\My Documents"',
        default=None,
        metavar="path",
        required=False,
    )
    parser.add_argument(
        "--ifexists",
        type=str.lower,
        choices=["uniquename", "skip", "overwrite", "prompt", "exit"],
        help='Action to take if file with same name already exists. Default is "%(default)s"',
        default="uniquename",
        required=False,
    )
    parser.add_argument(
        "--downloadhistory",
        type=str.lower,
        choices=["skipfiles", "ignore", "clear"],
        help="'skipfiles' means that files in history (ie, previously downloaded) will be skipped and not downloaded. Default is \"%(default)s\"",
        default="skipfiles",
        required=False,
    )
    parser.add_argument(
        "--onlyfolders",
        help='Only include image/file(s) existing in specified camera folders.. Ex: "--onlyfolders 100D7200 101D7200". Default is to include all folders',
        default=None,
        nargs="+",
        metavar="camera_folder",
        required=False,
    )
    parser.add_argument(
        "--excludefolders",
        help='Exclude image/file(s) existing in specified camera folders.. Ex: "--excludefolders 103D7200". Default is no exclusions.',
        default=None,
        nargs="+",
        metavar="camera_folder",
        required=False,
    ...
    parser.add_argument(
        "--filenamespec",
        type=str,
        help="Optionally rename files using dynamic renaming engine. See online help for documentation on 'spec'",
        default=None,
        metavar="spec",
        required=False,
    )
    parser.add_argument(
        "--dirnamespec",
        type=str,
        help="Optionally name directories using dynamic renaming engine. See online help for documentation on 'spec'",
        default=None,
        metavar="spec",
        required=False,
    )
    parser.add_argument(
        "--transferorder",
        type=str.lower,
        choices=["oldestfirst", "newestfirst"],
        help='Transfer oldest or newest files first. Default is "%(default)s"',
        default="oldestfirst",
        required=False,
    )
    parser.add_argument(
        "--slot",
        type=str.lower,
        help='Card slot on camera to read from. Default is "%(default)s", which means first populated slot',
        choices=["firstfound", "first", "second", "both"],
        default="firstfound",
        required=False,
    )
    parser.add_argument(
        "--cameratransferlist",
        type=str.lower,
        choices=["useifavail", "exitifnotavail", "ignore"],
        help='Decide how to handle images selected on camera. Default is "%(default)s"',
        default="useifavail",
        required=False,
    )
    parser.add_argument(
        "--downloadexec",
        help="Launch application for each file downloaded",
        default=None,
        nargs="+",
        metavar=("executable", "arguments"),
        required=False,
    )
    parser.add_argument(
        "--downloadexec_extlist",
        help="Type of files(s) by extension on wich to perform --downloadexec on. Default is all file types",
        default=None,
        nargs="+",
        metavar="extension",
        required=False,
    ...
    parser.add_argument(
        "--downloadexec_options",
        help="Options for launcing application. For example 'wait' waits for launched app to exit before proceeding to next download. See online help for more options",
        default=[],
        nargs="+",
        metavar="option",
        required=False,
    ...
    parser.add_argument(
        "--realtimepollsecs",
        type=int,
        help="How often camera is polled for new images in realtime mode, in seconds. Default is every %(default)s seconds",
        default=3,
        metavar="seconds",
        required=False,
    )
    parser.add_argument(
        ...
        help='Sets how much information is saved to the result log. Default is "%(default)s"',
        default="normal",
        required=False,
    )
    ...
    parser.add_argument(
        "--mtpobjcache",
        type=str.lower,
        choices=["enabled", "writeonly", "readonly", "verify", "disabled"],
        help=argparse.SUPPRESS,
        default="enabled",
        required=False,
    )
    parser.add_argument(
        "--mtpobjcache_maxagemins", help=argparse.SUPPRESS, type=int, default=0, required=False
    )  # default is 0=indefinite (never invalidate based on age)
    parser.add_argument(
        "--maxgetobjtransfersizekb",
        help=argparse.SUPPRESS,
        type=int,
        default=DEFAULT_MAX_KB_PER_GET_OBJECT_REQUEST,
        required=False,
    )
    parser.add_argument(
        "--maxgetobjbuffersizekb",
        help=argparse.SUPPRESS,
        type=int,
        default=DEFAULT_MAX_KB_TO_BUFFER_FOR_GET_OBJECT_REQUESTS,
        required=False,
    )
    parser.add_argument(
        "--initcmdreq_guid",
        help=argparse.SUPPRESS,
        type=str.lower,
        default="0x7766554433221100-0x0000000000009988",
        required=False,
    )  # GUID order in string is high-low
    parser.add_argument("--initcmdreq_hostname", help=argparse.SUPPRESS, type=str, default="airmtp", required=False)
    parser.add_argument(
        "--initcmdreq_hostver", help=argparse.SUPPRESS, type=conver_int_auto_radix, default=0x00010000, required=False
    )
    parser.add_argument(
        "--opensessionid", help=argparse.SUPPRESS, type=conver_int_auto_radix, default=None, required=False
    )
    parser.add_argument(
        "--maxclockdeltabeforesync", help=argparse.SUPPRESS, type=str.lower, default="5", required=False
    )
    parser.add_argument(
        "--camerasleepwhendone",
        help=argparse.SUPPRESS,
    ...
    parser.add_argument(
        "--sonyuniquecmdsenable",
        help=argparse.SUPPRESS,
        type=conver_int_auto_radix,
        default=SONY_UNQIUECMD_ENABLE_SENDING_MSG,
        required=False,
    )
    parser.add_argument(
        "--suppressdupconnecterrmsgs",
        help=argparse.SUPPRESS,
        type=str.lower,
        choices=["no", "yes"],
        default="yes",
    ...
    parser.add_argument(
        "--rtd_mtppollingmethod_newobjdetection",
        help=argparse.SUPPRESS,
        type=str.lower,
        choices=["objlist", "numobjs"],
        default="objlist",
        required=False,
    )
    parser.add_argument(
        "--rtd_maxsecsbeforeforceinitialobjlistget", help=argparse.SUPPRESS, type=int, default=5, required=False
    )
    parser.add_argument("--ssdp_discoveryattempts", help=argparse.SUPPRESS, type=int, default=3, required=False)
    parser.add_argument(
        "--ssdp_discoverytimeoutsecsperattempt", help=argparse.SUPPRESS, type=int, default=2, required=False
    )
    parser.add_argument(
        "--ssdp_discoveryflags", help=argparse.SUPPRESS, type=conver_int_auto_radix, default=None, required=False
    )
    ...
    if os.path.exists(defaultArgFilename):
        sys.argv.insert(
            1, "!" + defaultArgFilename
        )  # insert as first arg (past script name), so that the options in the file can still be overriden by user-entered cmd line options
    ...
    g.fileTransferOrder = (
        FILE_TRANSFER_ORDER_OLDEST_FIRST
        if g.args["transferorder"] == "oldestfirst"
        else FILE_TRANSFER_ORDER_NEWEST_FIRST
    )
    ...
    if g.args["logginglevel"] == "normal":
        ...
        applog_set_loggingFlags(
            APPLOGF_LEVEL_INFORMATIONAL | APPLOGF_LEVEL_ERROR | APPLOGF_LEVEL_WARNING | APPLOGF_LEVEL_VERBOSE
        )
    elif g.args["logginglevel"] == "debug":
        applog_set_loggingFlags(
            APPLOGF_LEVEL_INFORMATIONAL
            | APPLOGF_LEVEL_ERROR
            | APPLOGF_LEVEL_WARNING
            | APPLOGF_LEVEL_VERBOSE
            | APPLOGF_LEVEL_DEBUG
        )
...
def mtpCountedUtf16ToPythonUnicodeStr(data):
    ...
    unicodeStr = str(data[1 : 1 + utf16ByteLenIncludingNull - 2], "utf-16")

    # some Nikon strings have trailing NULLs for padding - remove them
    for charPos in reversed(range(len(unicodeStr))):
...
def parseMtpCountedList(data, elementSizeInBytes):
    ...
    offset = 4
    for entryIndex in range(countEntries):
        (entry,) = struct.unpack(
            "<" + elementSizeToUnpackStr[elementSizeInBytes], data[offset : offset + elementSizeInBytes]
        )
...
def parseMtpDeviceInfo(data):
    ...
    serialNumberStr = removeLeadingCharsFromStr(
        serialNumberStr, {" ", "0"}
    )  # remove leading spaces/zeros from serial number

    return MtpDeviceInfoTuple(
        standardVersion,
        vendorExtensionID,
        vendorExtensionVersion,
        vendorExtensionDescStr,
        set(operationsSupportedList),
        set(eventsSupportedList),
        set(devicePropertiesSupportedList),
        set(captureFormatsSupportedList),
        set(imageFormatsSupportedList),
        manufacturerStr,
        modelStr,
        deviceVersionStr,
        serialNumberStr,
...
def convertGuidStrToLongs(guidHexStr):
    if guidHexStr.find(":") != -1:
        ...
        for nthField in range(len(macAddressFieldList)):
...
def getMtpObjectInfo(objHandle, fRetryErrors=True):
    ...
    while True:
        try:
            ...
            applog_d(
                "getMtpObjectInfo() failed: {:s}, elaspsedTimeSecs={:.2f}".format(
                    getMtpRespDesc(e.mtpRespCode), elapsedTimeSecs
                )
            )
            if elapsedTimeSecs >= 5:
                raise
            time.sleep(0.25)  # wait 1/4 second before retrying
...
def ssdpDiscoverCameraIpAddress():
    ...
    #
    ssdpServiceNames = [
        "urn:microsoft-com:service:MtpNullService:1",  # Sony
    ...
    try:
        ssdpMessage = ssdp.discover(
            ssdpServiceNames,
            lambda ssdpMessage: ssdp.extractIpAddressFromSSDPMessage(ssdpMessage) != None,
            g.args["ssdp_discoveryattempts"],
            g.args["ssdp_discoverytimeoutsecsperattempt"],
            g.args["ssdp_discoveryflags"],
            g.args["ssdp_addmulticastif"],
        )
        if ssdpMessage == None:
        ...
        raise ssdp.DiscoverFailureException(
            ">> Connection Failed <<\n\n" + str(e)
        )  # prepend "Connection Failed" message to exception text
    finally:
        consoleClearLine()
...
def startMtpSession():
    ...
    #
    g.socketPrimary = mtpwifi.openConnection(
        ipAddressStr, True, g.args["connecttimeout"], g.args["socketreadwritetimeout"]
    )

    #
    # get session ID
    #
    data = mtpwifi.sendInitCmdReq(
        g.socketPrimary, (guidHigh, guidLow), g.args["initcmdreq_hostname"], g.args["initcmdreq_hostver"]
    )
    ...
    #
    g.socketEvents = mtpwifi.openConnection(
        ipAddressStr, False, g.args["connecttimeout"], g.args["socketreadwritetimeout"]
    )
    ...
    while True:
        try:
            ...
            if not fSessionIdFromCmdLine and e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR:
                applog_d(
                    "MTP_OP_OpenSession failed: {:s} for sessionID 0x{:08x}".format(
                        getMtpRespDesc(e.mtpRespCode), g.sessionId
                    )
                )
                if g.sessionId != 0x1:
...
def endMtpSession():
    ...
    if secsElapsedSinceOpenSession < 1:
        applog_d(
            "endMtpSession(): Delaying for {:.2f} seconds to work around Nikon bug".format(
                1 - secsElapsedSinceOpenSession
            )
        )
...
def getMtpDeviceInfo():
    ...
    if g.mtpDeviceInfo:
        ...
        if mtpDeviceInfo != g.mtpDeviceInfo:
            applog_e(
                'Discovered different camera during retry invocation. Orig camera was Model "{:s}", S/N ":{:s}", New camera is "{:s}", S/N ":{:s}"'.format(
                    g.mtpDeviceInfo.modelStr,
                    g.mtpDeviceInfo.serialNumberStr,
                    mtpDeviceInfo.modelStr,
                    mtpDeviceInfo.serialNumberStr,
                )
            )
    ...
    #
    g.cameraLocalMetadataPathAndRootName = os.path.join(
        g.appDataDir, "{:s}-SN{:s}".format(g.mtpDeviceInfo.modelStr, g.mtpDeviceInfo.serialNumberStr)
    )
...
def selectMtpStorageId():
    ...
    for i in range(0, len(mtpStorageIds.storageIdsList)):
    ...
    firstCardPresent = -1
    countCardsPresent = 0
    for i in range(0, countCardSlots):
    ...
    applog_d(
        "  storageId to be used for this invocation: {:08x} [cardsPresentBitmap=0x{:04x}]".format(
            storageId, cardsPresentBitmap
        )
    )
    ...
    if g.storageId != MTP_STORAGEID_ALL_CARDS:
        ...
        for i in range(0, countCardSlots):
            ...
            if storageId & MTP_STORAGEID_PresenceBit:
                mtpTcpCmdResult = mtpwifi.execMtpOp(
                    g.socketPrimary, MTP_OP_GetStorageInfo, struct.pack("<I", storageId)
                )
                mtpStorageInfo = parseMtpStorageInfo(mtpTcpCmdResult.dataReceived)
                applog_d(mtpStorageInfo)
                g.mtpStorageInfoList.append(mtpStorageInfo)
...
def saveMtpObjectsToDiskCache():
    ...
    for nObjInex in range(0, countObjs):
    ...
    hashOfMtpObjectInfoCacheTuple = hashlib.sha512(
        mtpObjectInfoCacheAsPickleBytes
    ).hexdigest()  # generate a SHA-512 hash digest, represented in hex ASCII
    ...
    try:
        ...
        fMtpObjectCache.write(
            mtpObjectInfoCacheAsPickleBytes
        )  # write the tuple. note we're using the already-pickled data instead of calling dump(). this is for performance
        ...
        if fMtpObjectCache:
            fMtpObjectCache.close()
        applog_e(
            "I/O error writing obj hash to {:s}, cache will not be available next session: {:s}".format(
                mtpObjectInfoCacheFilename, str(e)
            )
        )
        deleteFileIgnoreErrors(
            mtpObjectInfoCacheFilename
        )  # delete cache in case some (partial) data was written before exception
        ...
        if fMtpObjectCache:
            fMtpObjectCache.close()
        applog_e(
            "Non-I/O error generating obj hash to {:s}, cache will not be saved.".format(mtpObjectInfoCacheFilename)
        )
        deleteFileIgnoreErrors(
            mtpObjectInfoCacheFilename
        )  # delete cache in case some (partial) data was written before exception
    ...
    if os.path.exists(mtpObjectInfoCacheFilename):
        ...
        try:
            ...
            if fMtpObjectCache:
                fMtpObjectCache.close()
            applog_e(
                "Non-I/O error reading obj hash from {:s}. Cache will be discarded. (err={:s})".format(
                    mtpObjectInfoCacheFilename, str(sys.exc_info()[0])
                )
            )
        ...
        applog_v(
            "MTP Object cache has {:d} objects, age is {:s}".format(
                len(mtpObjectInfoCacheTuple.mtpObjectInfoList), str(datetime.timedelta(seconds=cacheAgeSeconds))
            )
        )
        if isDebugLog():
            applog_d("MTP object cache entries [count={:d}]".format(len(mtpObjectInfoCacheTuple.mtpObjectInfoList)))
            for i in range(len(mtpObjectInfoCacheTuple.mtpObjectInfoList)):
                applog_d(
                    "Entry {:4d}, handle = 0x{:08x}: {:s}".format(
                        i, mtpObjectInfoCacheTuple.objHandlesList[i], str(mtpObjectInfoCacheTuple.mtpObjectInfoList[i])
                    )
                )
        ...
        if g.args["mtpobjcache_maxagemins"] != 0 and cacheAgeSeconds / 60 >= g.args["mtpobjcache_maxagemins"]:
            applog_v(
                "Discarding MTP object cache due to age (max age is {:s})".format(
                    str(datetime.timedelta(minutes=g.args["mtpobjcache_maxagemins"]))
                )
            )
            return None
...
def loadAndValidateMtpObjectInfoCacheFromDisk(objHandlesFromCameraList):
    ...
    for nObjIndex in range(0, len(mtpObjectInfoCacheTuple.mtpObjectInfoList)):
        ...
        if objHandle not in objHandlesFromCameraSet:
            applog_d(
                'MTP obj handle 0x{:08x} for cache directory object "{:s}" does not exist'.format(
                    objHandle, cachedMtpObjectInfo.filename
                )
            )
        ...
        try:
            applog_d(
                'Validating MTP obj cache directory object "{:s}" on handle 0x{:08x}'.format(
                    cachedMtpObjectInfo.filename, objHandle
                )
            )
            ...
            #
            applog_d(
                'MTP object cache validation of "{:s}" resulted in {:s}'.format(cachedMtpObjectInfo.filename, str(e))
            )
            bInvalidateCache = True
            break
...
#
def createMtpObjectFromHandle(
    objHandle,
    createMtpObjectStatsStruct=None,
    cachedMtpObjectInfoListDict=None,
    fFindAndCreateAntecendentDirs=True,
    mtpObjectInfo=None,
):
    ...
    if mtpObj:
            ...
            if fIsInCache and g.args["mtpobjcache"] != "verify":
                ...
                if fIsInCache:
                    ...
                    if mtpObjectInfo != cachedMtpObjectInfoListDict[objHandle]:
                        applog_e(
                            'Found MTP object cache mismatch for "{:s}" vs  "{:s}"'.format(
                                mtpObjectInfo.filename, cachedMtpObjectInfoListDict[objHandle].filename
                            )
                        )
        ...
        #
        if (
            fFindAndCreateAntecendentDirs
            and mtpObjectInfo.parentObject
            and MtpObject.getByMtpObjectHandle(mtpObjectInfo.parentObject) == None
        ):
            applog_d(
                'Recursing to get parent dir of "{:s}" - objHandle=0x{:08x}, parent=0x{:08x}'.format(
                    mtpObjectInfo.filename, objHandle, mtpObjectInfo.parentObject
                )
            )
            createMtpObjectFromHandle(
                mtpObjectInfo.parentObject,
                createMtpObjectStatsStruct,
                cachedMtpObjectInfoListDict,
                fFindAndCreateAntecendentDirs,
            )
...
#
def createMtpObjectsFromHandleList(
    objHandlesList,
    createMtpObjectStatsStruct=None,
    cachedMtpObjectInfoListDict=None,
    fFindAndCreateAntecendentDirs=True,
    ...
    for nObjIndex in range(0, numObjectHandles):
        consoleWriteLine(
            "\rRetrieving list of images/files from camera: {:d}/{:d}     ".format(nObjIndex, numObjectHandles)
        )
        createMtpObjectFromHandle(
            objHandlesList[nObjIndex],
            createMtpObjectStatsStruct,
            cachedMtpObjectInfoListDict,
            fFindAndCreateAntecendentDirs,
        )
...
def buildMtpObjects(fReportObjectTotals=True):
    ...
    if g.args["cameratransferlist"] != "ignore":
        if g.fRealTimeDownloadPhaseStarted:
            raise AssertionError(
                "buildMtpObjects(): Checking for transfer list but already entered realtime download phase!"
            )
        #
        ...
        if MTP_OP_GetTransferList in g.mtpDeviceInfo.operationsSupportedSet or g.cameraMake == CAMERA_MAKE_NIKON:
            try:
                ...
                applog_i(
                    "Transfer list detected in camera - {:d} images/file(s) will be downloaded".format(
                        numObjectHandlesToGet
                    )
                )
            except mtpwifi.MtpOpExecFailureException as e:
                #
    ...
    if isDebugLog():
        ...
        applog_d(
            strutil.hexdump(
                struct.pack("<" + "I" * countFullObjHandlesList, *fullObjHandlesList),
                bytesPerField=4,
                includeASCII=False,
            )
        )
        if g.fAllObjsAreFromCameraTransferList:
            ...
            applog_d(
                strutil.hexdump(
                    struct.pack("<" + "I" * numObjectHandlesToGet, *objHandlesListToGet),
                    bytesPerField=4,
                    includeASCII=False,
                )
            )
    ...
    if fReportObjectTotals or isDebugLog():  # always report object totals when debug logging is enabled
        applog_i(
            "Processed info for {:d} files/dirs [{:d} from object cache, {:d} previous]".format(
                createMtpObjectStatsStruct.countObjectsProcessed,
                createMtpObjectStatsStruct.countCacheHits,
                createMtpObjectStatsStruct.countMtpObjectsAlreadyExisting,
            )
        )
    ...
    if g.fAllObjsAreFromCameraTransferList:
        if (
            createMtpObjectStatsStruct.countCacheHits < createMtpObjectStatsStruct.countObjectsProcessed
            and createMtpObjectStatsStruct.countObjectsProcessed >= countObjectsInCache / 2
        ):
            ...
            if g.args["downloadhistory"] == "clear":
                ...
                try:
                    with open(downloadHistoryFilename) as f:
                        downloadHistoryDict_Temp = {
                            "{:s}".format(line[0 : line.index("::::")]): "{:s}".format(
                                line[line.index("::::", 0) + 4 : line.rindex("\n")]
                            )
                            for line in f
                        }
                    ...
                    applog_v(
                        'Download history file "{:s}" loaded - {:d} entries'.format(
                            downloadHistoryFilename, len(downloadHistoryDict)
                        )
                    )
                except ValueError:
                    applog_e(
                        'Download history file "{:s}" appears corrupt and will be ignored and deleted'.format(
                    ...
                    applog_e(
                        'Error openg/reading download history file "{:s}". No history will be available this session.'.format(
                            downloadHistoryFilename
                        )
                    )
        else:
...
def writeDataToDownloadedFile(fileHandleIfAlreadyOpen, filenameWithPath, data, bCloseAfterWriting, bIsAppending):
    applog_d(
        "{:s} writing 0x{:x} bytes, closeAfterWriting={:d}".format(filenameWithPath, len(data), bCloseAfterWriting)
    )
    #
...
def doesMtpObjectPassUserFileFilter(mtpObject, fPrintFilterAction=True):
    ...
    if (
        mtpObject.mtpObjectInfo.objectFormat == MTP_OBJFORMAT_Assocation
        or mtpObject.mtpObjectInfo.objectFormat == MTP_OBJFORMAT_NONE
    ):
        if fPrintFilterAction:
            applog_d(
                "Skipping {:s} - object is not file - {:s}".format(
                    mtpObject.mtpObjectInfo.filename, getMtpObjFormatDesc(mtpObject.mtpObjectInfo.objectFormat)
                )
            )
        return False
    ...
    if g.args["extlist"] and isMtpFilenameExtInList(mtpObject.mtpObjectInfo.filename, g.args["extlist"]) == False:
        if fPrintFilterAction:
            applog_v(
                "Skipping {:s} - filename extension not in user-specified list".format(mtpObject.mtpObjectInfo.filename)
            )
        return False
    ...
    if g.objfilter_dateStartEpoch != None and mtpObject.captureDateEpoch < g.objfilter_dateStartEpoch:
        ...
        if fPrintFilterAction:
            applog_v(
                "Skipping {:s} - has capture date earlier than user-specified start date filter".format(
                    mtpObject.mtpObjectInfo.filename
                )
            )
        return False
    if g.objfilter_dateEndEpoch != None and mtpObject.captureDateEpoch > g.objfilter_dateEndEpoch:
        ...
        if fPrintFilterAction:
            applog_v(
                "Skipping {:s} - has capture date later than user-specified end date filter".format(
                    mtpObject.mtpObjectInfo.filename
                )
            )
        return False
    ...
    if g.args["onlyfolders"]:
        ...
        if (cameraFolder == "" and ("<ROOT>" not in g.args["onlyfolders"])) or cameraFolder not in g.args[
            "onlyfolders"
        ]:
            # image is in root directory of camera and "<ROOT>" not in list, or image is in directory not in list
            if fPrintFilterAction:
                applog_v(
                    "Skipping {:s}\\{:s} - folder not in --onlyfolders".format(
                        cameraFolder, mtpObject.mtpObjectInfo.filename
                    )
                )
            return False
    if g.args["excludefolders"]:
        ...
        if (cameraFolder == "" and "<ROOT>" in g.args["excludefolders"]) or cameraFolder in g.args["excludefolders"]:
            ...
            if fPrintFilterAction:
                applog_v(
                    "Skipping {:s}\\{:s} - folder in --excludefolders".format(
                        cameraFolder, mtpObject.mtpObjectInfo.filename
                    )
                )
            return False
...
def getNextUserFilteredMtpFileObject(prevObject, fileTransferOrder=FILE_TRANSFER_ORDER_USER_CONFIGURED):
    ...
    if prevObject == -1:
        mtpObject = (
            MtpObject.getNewest() if fileTransferOrder == FILE_TRANSFER_ORDER_NEWEST_FIRST else MtpObject.getOldest()
        )
    else:
        mtpObject = (
            prevObject.getOlder() if fileTransferOrder == FILE_TRANSFER_ORDER_NEWEST_FIRST else prevObject.getNewer()
        )
    while mtpObject != None and not doesMtpObjectPassUserFileFilter(mtpObject):
        mtpObject = (
            mtpObject.getOlder() if fileTransferOrder == FILE_TRANSFER_ORDER_NEWEST_FIRST else mtpObject.getNewer()
        )
    return mtpObject
...
def genRenameDictKeysCommonToAllMtpObjects():
    ...
    renameDict["cameramake"] = g.mtpDeviceInfo.manufacturerStr.split(" ")[
        0
    ]  # just use the first word. for example, Nikon returns "Nikon Corporation"
...
#
def updateRenameDictKeysSpecificToMtpObject(
    renameDict, mtpObject, dlnum, dlnum_lifetime, localFilename=None, path=None
):
...
def doDownloadExec(renameDict, mtpObject):
    ...
    if (
        g.args["downloadexec_extlist"]
        and isMtpFilenameExtInList(mtpObject.mtpObjectInfo.filename, g.args["downloadexec_extlist"]) == False
    ):
        return
    ...
    for arg in g.args["downloadexec"]:
        ...
        if renamedArg != "":
            if "notildereplacement" not in g.args["downloadexec_options"]:
                ...
                if (
                    execArgs
                ):  # only do this replacement on args that aren't the first, to allow tidles in application/script name
            ...
            if not execArgs:
                ...
                applog_v(
                    "Skipping 'downloadexec' for \"{:s}\" due to empty first argument of rename string".format(
                        renameDict["filename"]
                    )
                )
                break
    ...
    if execArgs:
        ...
        try:
            ...
            errStr = "Error launching for 'downloadexec' {:s}: {:s} {:s}".format(
                str(execArgs), str(sys.exc_info()[0]), str(sys.exc_info()[1])
            )
        ...
        if "wait" in g.args["downloadexec_options"]:
            ...
            while process.poll() == None:
                ...
                time.sleep(0.10)  # sleep for 100ms between each process completion check
            retCode = process.wait()
            if "exitonfailcode" in g.args["downloadexec_options"]:
                if retCode != 0:
                    applog_i(
                        "Exiting due to non-zero return code ({:d}) of launched app for '--downloadexec'".format(
                            retCode
                        )
                    )
...
def respErrorDuringDownload_checkIfFileDeletedOnCamera(mtpObject, localFilename, mtpOpExecFailureException):
    ...
    #
    applog_d(
        'RESP error downloading "{:s}": {:s}'.format(
            localFilename, getMtpRespDesc(mtpOpExecFailureException.mtpRespCode)
        )
    )
    try:
        ...
        mtpRespCodesIndicatingFileDeleted = [
            MTP_RESP_InvalidObjectHandle,
            MTP_RESP_OperationNotSupported,
            MTP_RESP_ParameterNotSupported,
            MTP_RESP_AccessDenied,
            MTP_RESP_PartialDeletion,
            MTP_RESP_NoValidObjectInfo,
            MTP_RESP_InvalidParentObject,
            MTP_RESP_InvalidParameter,
        ]
        if e.mtpRespCode in mtpRespCodesIndicatingFileDeleted:
            applog_e(
                'Error downloading "{:s}" - file appears to have been deleted in-camera by user - ignoring ({:s}/{:s})'.format(
                    localFilename, getMtpRespDesc(mtpOpExecFailureException.mtpRespCode), getMtpRespDesc(e.mtpRespCode)
                )
            )
            return True
...
#
def downloadMtpFileObjects_DownloadProgressCallback(
    bytesReceivedAllCurrentPayloads,
    totalBytesExpectedAllCurrentPayloads,
    bytesReceivedPriorPayloads,
    totalFileSizeIfKnown,
):
    if not hasattr(downloadMtpFileObjects_DownloadProgressCallback, "lastPctPrinted"):
        downloadMtpFileObjects_DownloadProgressCallback.lastPctPrinted = (
            -1
        )  # static var to track last percentage printed, to avoid unnecessary updates
...
def downloadMtpFileObjects(firstMtpObjectToDownload=None):
    ...
    fFirstLoopIteration = (
        True  # unfortunate hack because Python doesn't support do-while or post conditions on for loops
    )
    while True:
        ...
        if fUsingRenameEngineForAnyParameter:
            ...
            updateRenameDictKeysSpecificToMtpObject(
                renameDict,
                mtpObject,
                g.countFilesDownloadedPersistentAcrossStatsReset,
                len(downloadHistoryDict),
                filenameWithObjTypeSuffixBeforeRename,
            )
        ...
        #
        downloadHistoryDescStr_Key = "{:s}::{:s}::{:,}".format(
            filenameWithObjTypeSuffixBeforeRename,
            mtpObject.mtpObjectInfo.captureDateStr,
            mtpObject.mtpObjectInfo.objectCompressedSize,
        )
        if fSkipDownloadedFiles and downloadHistoryDescStr_Key in downloadHistoryDict:
            ...
            listDownloadHistory = str.split(
                downloadHistoryDict[downloadHistoryDescStr_Key], "::"
            )  # each field of the additional info area is separated by two colons
            downloadHistoryElementTuple = DownloadHistoryElement(listDownloadHistory[0], listDownloadHistory[1])
            applog_v(
                'Skipping "{:s}" - downloaded on {:s} to "{:s}" '.format(
                    filenameWithObjTypeSuffixBeforeRename,
                    downloadHistoryElementTuple.dateDownloadedStr,
                    downloadHistoryElementTuple.pathDownloadedToStr,
                )
            )
        ...
        if mtpObject.partialDownloadObj().getLocalFilenameWithoutPath() == None:
            ...
            if os.path.exists(localFilenameWithPath):
                if g.args["ifexists"] == "prompt":
                    ...
                    #
                    keyResponse = promptWithSingleKeyResponse(
                        "(S)kip, (O)verwrite, (U)niquename, (E)xit [+enter]: ", "soue"
                    )
                else:
                    keyResponse = ""
                if g.args["ifexists"] == "skip" or keyResponse == "S":
                    ...
                    if not keyResponse:
                        applog_v(
                            '"{:s}" exists - will write to "{:s}"'.format(
                                localFilenameWithPath, uniqueFilenameWithoutPath
                            )
                        )
        ...
        if g.fAllObjsAreFromCameraTransferList:
            mtpwifi.execMtpOp(
                g.socketPrimary, MTP_OP_NotifyFileAcquisitionStart, struct.pack("<I", mtpObject.mtpObjectHandle)
            )
        ...
        if bUsingGetPartialObject:
            ...
            try:
                ...
                if bytesWritten > 0:
                    ...
                    #
                    applog_d(
                        "{:s} - resuming download - bytesWritten=0x{:x}, fileSize=0x{:x}".format(
                            localFilenameWithoutPath, bytesWritten, fileSizeBytes
                        )
                    )
                    if os.path.exists(localFilenameWithPath_TemporaryFilename):
                        ...
                        if statInfo.st_size != bytesWritten:
                            raise AssertionError(
                                "About to resume download for \"{:s}\" but it's filesize is not equal the how much data we've already downloaded and written (expected 0x{:x}, actual 0x{:x}".format(
                                    localFilenameWithoutPath, bytesWritten, statInfo.st_size
                                )
                            )
                    else:
                        raise AssertionError(
                            'About to resume download for "{:s}" but the file is missing. It should be present and have a size of 0x{:x}'.format(
                                localFilenameWithoutPath, bytesWritten
                            )
                        )
                ...
                while offsetIntoImage < fileSizeBytes:
                    ...
                    if len(dataReceived) + bytesToDownloadThisPiece > g.maxGetObjBufferSize:
                        ...
                        bytesToDownloadThisPiece -= (
                            len(dataReceived) + bytesToDownloadThisPiece
                        ) - g.maxGetObjBufferSize
                    if bytesToDownloadThisPiece == 0:
                        # shouldn't happen
                        raise AssertionError(
                            "bytesToDownloadThisPiece is zero! offset=0x{:x}, fileSize=0x{:x}, dataReceived=0x{:x}, max=0x{:x}/0x{:x}".format(
                                offsetIntoImage,
                                fileSizeBytes,
                                len(dataReceived),
                                g.maxGetObjTransferSize,
                                g.maxGetObjBufferSize,
                            )
                        )
                    applog_d(
                        "{:s} - downloading next piece, offset=0x{:x}, count=0x{:x}".format(
                            localFilenameWithoutPath, offsetIntoImage, bytesToDownloadThisPiece
                        )
                    )
                    timeStart = secondsElapsed(None)
                    mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(
                        g.socketPrimary,
                        MTP_OP_GetPartialObject,
                        struct.pack("<III", mtpObject.mtpObjectHandle, offsetIntoImage, bytesToDownloadThisPiece),
                        rxTxProgressFunc=lambda bytesReceivedAllCurrentPayloads, totalBytesExpectedAllCurrentPayloads: downloadMtpFileObjects_DownloadProgressCallback(
                            bytesReceivedAllCurrentPayloads,
                            totalBytesExpectedAllCurrentPayloads,
                            offsetIntoImage,
                            fileSizeBytes,
                    ...
                    if bIsFinalPiece or len(dataReceived) >= g.maxGetObjBufferSize:
                        ...
                        foDownloadedFile = writeDataToDownloadedFile(
                            foDownloadedFile, localFilenameWithPath, dataReceived, bIsFinalPiece, (bytesWritten != 0)
                        )  # close file if this is the last piece
                ...
                if (
                    e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR
                    and respErrorDuringDownload_checkIfFileDeletedOnCamera(mtpObject, localFilenameWithoutPath, e)
                ):
                ...
                if fFileDeletedOnCamera == False:
                    ...
                    mtpObject.partialDownloadObj().addDownloadTimeSecs(secondsElapsed(timeStart))
                    applog_d(
                        "{:s} - error during download, writing 0x{:x} bytes of buffered data".format(
                            localFilenameWithoutPath, len(dataReceived)
                        )
                    )
                    ...
                    if dataReceived:
                        foDownloadedFile = writeDataToDownloadedFile(
                            foDownloadedFile, localFilenameWithPath, dataReceived, False, (bytesWritten != 0)
                        )
                        bytesWritten += len(dataReceived)
                        mtpObject.partialDownloadObj().addBytesWritten(len(dataReceived))
                    if e.partialData:
                        ...
                        #
                        applog_d(
                            "{:s} - writing partial payload data of 0x{:x} bytes".format(
                                localFilenameWithoutPath, len(e.partialData)
                            )
                        )
                        foDownloadedFile = writeDataToDownloadedFile(
                            foDownloadedFile, localFilenameWithPath, e.partialData, False, (bytesWritten != 0)
                        )
            ...
            try:
                mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(
                    g.socketPrimary,
                    mtpOpGet,
                    struct.pack("<I", mtpObject.mtpObjectHandle),
                    rxTxProgressFunc=lambda bytesReceivedAllCurrentPayloads, totalBytesExpectedAllCurrentPayloads: downloadMtpFileObjects_DownloadProgressCallback(
                        bytesReceivedAllCurrentPayloads, totalBytesExpectedAllCurrentPayloads, 0, 0
                    ),
                )
                dataReceived = mtpTcpCmdResultGetObj.dataReceived
        ...
        if not fFileDeletedOnCamera:
            ...
            os.rename(
                localFilenameWithPath_TemporaryFilename, localFilenameWithPath
            )  # download done - safe to rename to final filename
            ...
            #
            if (
                fileDownloadTimeSecs > 0
            ):  # avoid divide-by-zero, though not expecting ever expecting 'fileDownloadTimeSecs' to be zero
                thisFileDownloadRateMbSec = fileSizeBytes / fileDownloadTimeSecs / 1048576
            else:
                thisFileDownloadRateMbSec = 0
            applog_i(
                "{:s} [size = {:,}] in {:.2f} seconds ({:.2f} MB/s)".format(
                    localFilenameWithPath, fileSizeBytes, fileDownloadTimeSecs, thisFileDownloadRateMbSec
                )
            )
            ...
            if g.fAllObjsAreFromCameraTransferList:
                mtpwifi.execMtpOp(
                    g.socketPrimary, MTP_OP_NotifyFileAcquisitionEnd, struct.pack("<I", mtpObject.mtpObjectHandle)
                )
            ...
            if g.args["downloadexec"]:
                renameDict[
                    "filename"
                ] = localFilenameWithoutPath  # update dict in case filename changed due to ifexists unique creation
...
def printMtpObjectDirectoryListing():
    ...
    while mtpObject:
        ...
        if g.countCardsUsed > 1:
            ...
            fullPathStr = (
                "CARD{:d}\\".format(getSlotIndexFromStorageId(mtpObject.mtpObjectInfo.storageId)) + fullPathStr
            )
    ...
    applog_i(
        "        {:4d} Dir(s)  {:13,} bytes free {:s}".format(
            MtpObject._CountMtpObjectDirectories,
            g.mtpStorageInfoList[0].freeSpaceBytes,
            "[CARD 1]" if g.countCardsUsed > 1 else "",
        )
    )
    for cardIndex in range(1, g.countCardsUsed):
        applog_i(
            "                     {:13,} bytes free [CARD {:d}]".format(
                g.mtpStorageInfoList[cardIndex].freeSpaceBytes, cardIndex + 1
            )
        )
...
def getMtpDeviceProperty(mtpDevicePropCode, fIgnoreIfNotSupported=False):
    try:
        mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(
            g.socketPrimary, MTP_OP_GetDevicePropValue, struct.pack("<I", mtpDevicePropCode)
        )
...
def setMtpDeviceProperty(mtpDevicePropCode, devicePropData, fIgnoreIfNotSupported=False):
    try:
        mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(
            g.socketPrimary, MTP_OP_SetDevicePropValue, struct.pack("<I", mtpDevicePropCode), devicePropData
        )
        return False
    except mtpwifi.MtpOpExecFailureException as e:
        if fIgnoreIfNotSupported and e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR:
...
def setMtpDeviceProperty_Canon(devicePropData, fIgnoreIfNotSupported=False):
    try:
        mtpTcpCmdResultGetObj = mtpwifi.execMtpOp(
            g.socketPrimary, MTP_OP_Canon_SetDevicePropValue, dataToSend=devicePropData
        )
        return False
    except mtpwifi.MtpOpExecFailureException as e:
        if fIgnoreIfNotSupported and e.mtpRespCode != MTP_RESP_COMMUNICATION_ERROR:
...
def syncCameraDateTimeIfNecessary():
    ...
    if (
        g.cameraMake != CAMERA_MAKE_CANON
    ):  # we don't suppor retriving current time on Canon bodies, so we always set it unconditionally
        ...
        systemCurrentTimeEpoch = (
            time.time()
        )  # note that is some inherent skew between retrieving the camera's time vs getting system time
        ...
        applog_d(
            "Camera time epoch: {:.2f}, System time epoch: {:.2f}".format(
                cameraCurrentTimeEpoch, systemCurrentTimeEpoch
            )
        )
...
def printSpinningProgressCharToConsole():
    ...
    if not hasattr(
        printSpinningProgressCharToConsole, "lastProgressCharPrinted"
    ):  # static variable to track last progress character printed
...
def parseNikonMtpEventData(data):
    ...
    for nthEvent in range(eventCount):
...
def genNikonEventListDescription(mtpEventList):
    str = ""
    for nthEvent in range(len(mtpEventList)):
        ...
        if nthEvent > 0:
            str += "\n"
        str += "E#{:d}: {:s}, Param: {:s}".format(
            nthEvent, getMtpEventDesc(mtpEventList[nthEvent].eventCode), parameterDesc
        )
    ...
    try:
        while True:
            ...
            if nikonMtpEventList:
                ...
                for nthEvent in range(len(nikonMtpEventList)):
                    if nikonMtpEventList[nthEvent].eventCode == MTP_EVENT_ObjectAdded:
                        ...
                        if MtpObject.getByMtpObjectHandle(objHandle):
                            applog_d(
                                "realTimeCapture: MtpObject for handle 0x{:08x} already exists, skipping".format(
                                    objHandle
                                )
                            )
                            continue
                        ...
                        if g.storageId != MTP_STORAGEID_ALL_CARDS and mtpObjectInfo.storageId != g.storageId:
                            if isVerboseLog():
                                ...
                                fRedrawWaitingMessage = True
                                applog_v(
                                    'Ignoring "{:s}" because it\'s not from your configured --slot'.format(
                                        mtpObjectInfo.filename
                                    )
                                )
                        else:
        ...
        consoleClearLine()
        g.dlstats.reportDownloadStats(
            g.args["realtimedownload"] != "only"
        )  # print stats even if no files downloaded only if this was a realtime-only session
        raise
...
def realTimeCapture_MtpObjPollingMethod():
    ...
    if not lastFullMtpHandleList:
        ...
        if isDebugLog():
            applog_d(
                "realTimeCapture_MtpObjPollingMethod(): First MTP object list (count={:d}):".format(
                    len(lastFullMtpHandleList)
                )
            )
            applog_d(
                strutil.hexdump(
                    struct.pack("<" + "I" * len(lastFullMtpHandleList), *lastFullMtpHandleList),
                    bytesPerField=4,
                    includeASCII=False,
                )
            )
    ...
    try:
        while True:
            ...
            if g.args["rtd_mtppollingmethod_newobjdetection"] == "numobjs":
                ...
                if fNumObjsChanged:
                    applog_d(
                        "fNumObjsChanged TRUE: (previous=0x{:d}, new=0x{:d}".format(
                            len(lastFullMtpHandleList), numMtpObjects
                        )
                    )
            ...
            if fNumObjsChanged or g.args["rtd_mtppollingmethod_newobjdetection"] == "objlist":
                ...
                if isDebugLog() and (fNumObjsChanged or newMtpHandleList):
                    applog_d(
                        "realTimeCapture_MtpObjPollingMethod(): Current MTP object list (count={:d}):".format(
                            len(currentFullMtpHandleList)
                        )
                    )
                    applog_d(
                        strutil.hexdump(
                            struct.pack("<" + "I" * len(currentFullMtpHandleList), *currentFullMtpHandleList),
                            bytesPerField=4,
                            includeASCII=False,
                        )
                    )
                    applog_d(
                        "realTimeCapture_MtpObjPollingMethod(): New MTP object list (count={:d}):".format(
                            len(newMtpHandleList)
                        )
                    )
                    applog_d(
                        strutil.hexdump(
                            struct.pack("<" + "I" * len(newMtpHandleList), *newMtpHandleList),
                            bytesPerField=4,
                            includeASCII=False,
                        )
                    )
...
def realTimeCapture():
    ...
    g.args[
        "cameratransferlist"
    ] = "ignore"  # so that recovery operations use the full camera object list rather than any transfer list already processed
    g.fAllObjsAreFromCameraTransferList = (
        False  # we'll be adding non-transfer list objects (ie, the files we download in realtime)
    )
    g.args[
        "transferorder"
    ] = "oldestfirst"  # so that downloadMtpFileObjects() will properly enumerate through multiple realtime images as we add them
    g.fRealTimeDownloadPhaseStarted = (
        True  # used by logic when making decisions on reporting for downloads, recovery operations, etc..
    )
...
def appMain():
    ...
    try:
        ...
        #
        if (
            (g.fRetrievedMtpObjects == False and g.args["realtimedownload"] != "only")
            or g.fRealTimeDownloadPhaseStarted
        ...
        if g.args["action"] == "listfiles":
            ...
            if g.fRetrievedMtpObjects:
                ...
                downloadMtpFileObjects()
                g.dlstats.reportDownloadStats(
                    g.fRealTimeDownloadPhaseStarted or g.args["realtimedownload"] == "only"
                )  # don't print stats if no files downloaded if we're in realtime phase or in a session recovery of a realtime-only session
        ...
        if e.mtpRespCode == MTP_RESP_COMMUNICATION_ERROR:
            ...
            return (
                ERRNO_CAMERA_UNEXPECTED_RESP_CODE,
                False,
            )  # don't retry on a high-level MTP error - we're trying to do something the camera doesn't like
        ...
        if (
            e.errno != 0
        ):  # make sure we're returning a non-zero exit code (not all socket exceptions can be relied on to do so)
            return (e.errno, True)
        else:
        ...
        )  # we always print stack frames for programming errors ('printstackframes' is  ignored)
...
def shutdownApplog():
    applog(
        ">>>> airmtpcmd session over - App Exit Time: {:s}".format(strutil.getDateTimeStr(fMilitaryTime=True)),
        APPLOGF_LEVEL_INFORMATIONAL | APPLOGF_DONT_WRITE_TO_CONSOLE,
    )  # used by utils to see if we exited gracefully
    applog_shutdown()
...
def main():
    ...
    #
    _errno = applog_init(
        APPLOGF_LEVEL_INFORMATIONAL | APPLOGF_LEVEL_ERROR,
        os.path.join(g.appDataDir, "airmtpcmd-log-last.txt"),
    ...
    applog_i(
        "Time: {:s}, Py: {:d}.{:d}.{:d}, OS: {:s}\n".format(
            strutil.getDateTimeStr(fMilitaryTime=True),
    ...
    while True:
        try:
            ...
            consoleWriteLine(
                "\rDelaying {:d} seconds before retrying. Press <ctrl-c> to exit [{:d} attempts]: ".format(
                    g.args["retrydelaysecs"], attemptNumber
                )
            )
            while secondsToNextRetry:
                ...
                time.sleep(1)
                consoleWriteLine(
                    "\b" * len(secondsToNextRetryStr)
                    + " " * len(secondsToNextRetryStr)
                    + "\b" * len(secondsToNextRetryStr)
                )
