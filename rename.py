...
def getNextSpecifierPos(formatString, formatStringPos):
    ...
    if nextSpecifierEndPos == -1:
        raise GenerateReplacementNameException(
            "Missing specifier after @ at character #{:d}".format(nextSpecifierStartPos)
        )
...
def performRename(formatString, parmsDict):
    ...
    specifierTranslationDict["capturedate_dow"] = str(
        captureTimeStruct.tm_wday + 1
    )  # date captured day of week (numeric, 1=Monday)
    specifierTranslationDict["capturedate_woy"] = time.strftime(
        "%W", captureTimeStruct
    )  # date captured week of year (numeric, Monday first day of week)
    specifierTranslationDict["capturedate_month"] = time.strftime("%B", captureTimeStruct)  # date captured month (text)
    specifierTranslationDict["capturedate_dayofweek"] = time.strftime(
        "%A", captureTimeStruct
    )  # date captured day of week (text)
    specifierTranslationDict["capturedate_season"] = dayOfYearToSeason(
        captureTimeStruct.tm_yday
    )  # date captured season (text)
    ...
    specifierTranslationDict["dldate_dow"] = str(
        downloadTimeStruct.tm_wday + 1
    )  # date downloaded day of week (numeric, 1=Monday)
    specifierTranslationDict["dldate_woy"] = time.strftime(
        "%W", downloadTimeStruct
    )  # date downloaded week of year (numeric, Monday first day of week)
    specifierTranslationDict["dldate_month"] = time.strftime("%B", downloadTimeStruct)  # date downloaded month (text)
    specifierTranslationDict["dldate_dayofweek"] = time.strftime(
        "%A", downloadTimeStruct
    )  # date downloaded day of week (text)
    specifierTranslationDict["dldate_season"] = dayOfYearToSeason(
        downloadTimeStruct.tm_yday
    )  # date downloaded season (text)
    ...
    specifierTranslationDict["filename_root"] = os.path.splitext(parmsDict["filename"])[
        0
    ]  # local filename base (filename without extension)
    specifierTranslationDict["filename_ext"] = os.path.splitext(parmsDict["filename"])[1][
        1:
    ]  # local filename extension
    specifierTranslationDict["capturefilename"] = parmsDict["capturefilename"]  # capture filename
    specifierTranslationDict["capturefilename_root"] = os.path.splitext(parmsDict["capturefilename"])[
        0
    ]  # capture filename base (filename without extension)
    specifierTranslationDict["capturefilename_ext"] = os.path.splitext(parmsDict["capturefilename"])[1][
        1:
    ...
    specifierTranslationDict["slotnumber"] = str(
        parmsDict["slotnumber"]
    )  # camera media slot # file was downloaded from
    ...
    specifierTranslationDict["dlnum_lifetime"] = "{:04d}".format(
        parmsDict["dlnum_lifetime"]
    )  # download number lifetime for this model/serial
    ...
    while formatStringPos < formatStringLen:
        ...
        #
        #
        specifierForReporting = formatString[
            nextSpecifierStartPos : nextSpecifierEndPos + 1
        ]  # for use in reporting errors, entire specifier in original case including enclosing @@
        specifierWithArgs = formatString[
            nextSpecifierStartPos + 1 : nextSpecifierEndPos
        ]  # the entire specifier with optional args included (everything between @@)
        ...
        if specifierWithArgsLowercase.find("replace", 0, 7) != -1:
            ...
            if len(replaceList) == 1:
                raise GenerateReplacementNameException(
                    "No search string specified after specifier {:s} at character #{:d}".format(
                        specifierForReporting, nextSpecifierStartPos
                    )
                )
            if len(replaceList) == 2:
                raise GenerateReplacementNameException(
                    "No replacement string specified after specifier {:s} at character #{:d}".format(
                        specifierForReporting, nextSpecifierStartPos
                    )
                )
            if len(replaceList) > 3:
                raise GenerateReplacementNameException(
                    "Too many fields for specifier {:s} at character #{:d}".format(
                        specifierForReporting, nextSpecifierStartPos
                    )
                )
            if replaceList[0] == "replace":
                ...
                raise GenerateReplacementNameException(
                    "Unknown specifier {:s} at character #{:d}".format(specifierForReporting, nextSpecifierStartPos)
                )
            continue
        ...
        if len(specifierListLowercase) > 4:
            raise GenerateReplacementNameException(
                "Too many subscripts to specifier {:s} at character #{:d}".format(
                    specifierForReporting, nextSpecifierStartPos
                )
            )
        ...
        if len(specifierListLowercase) >= 2:
            if specifierListLowercase[1]:
                try:
                    ...
                    raise GenerateReplacementNameException(
                        "First subscript value is not integer for specifier {:s} at character #{:d}".format(
                            specifierForReporting, nextSpecifierStartPos
                        )
                    )
        if len(specifierListLowercase) >= 3:
            if specifierListLowercase[2]:
                try:
                    ...
                    raise GenerateReplacementNameException(
                        "Second subscript value is not integer for specifier {:s} at character #{:d}".format(
                            specifierForReporting, nextSpecifierStartPos
                        )
                    )
        optionsLowercase = ""
...
def testModule():
    ...
    print(
        performRename(
            "date capture: @capturedate_dow@ = @capturedate_dayofweek@, Week of Year = @capturedate_woy@, Month = @Capturedate_month@, Season = @Capturedate_season@",
            renameDict,
        )
    )
    ...
    print(
        performRename(
            "date downloaded: @dldate_dow@ = @dldate_dayofweek@, Week of Year = @dldate_woy@, Month = @Dldate_month@, Season = @Dldate_season@",
            renameDict,
        )
    )
    ...
    print(
        performRename(
            "pf=@pf@, filename=@filename@, root=@filename_root@, ext=@filename_ext@, dir=@CAMERAFOLDER@", renameDict
        )
    )
    print(
        performRename(
            "capfilename=@capturefilename@, capturefilenameroot=@capturefilename_root@, capturefilenameext=@capturefilename_ext@, path=@PATH@",
            renameDict,
        )
    )
    ...
    print(
        isSpecifierInFormatString("this is a test @first@ @second@ @cameramake:343:434@ of this routine", "cameramake")
    )
    print(
        isSpecifierInFormatString("this is a test @first@ @second@ @cameramake@ of this routine@test@", "cameramakes")
    )
