...
def hexdump(data, bytesPerField=1, includeASCII=1):
    ...
    if (len(data) % bytesPerField) != 0:
        applog_w(
            "hexdump: size of data (0x{:04x}) is not a multiple of bytesPerField ({:d})".format(
                len(data), bytesPerField
            )
        )
        return strHexDump
    for offset in range(0, len(data), bytesPerField):
        ...
        strHexDump += "{:0{:d}x} ".format(
            thisField, bytesPerField * 2
        )  # (value,width) - width: bytes=2, halfwords=4, words=8
        ...
        if endingOffsetThisFieldInLine == 16 or (offset == len(data) - 1):
            ...
            if includeASCII:
                if endingOffsetThisFieldInLine < 16:
                    ...
                    strHexDump += " " * (
                        fieldsNotPrintedInFinalLine * charactersPerFieldIncludingSpace
                    )  # add spaces for each missing field
                    if endingOffsetThisFieldInLine < 8:
                        strHexDump += "  "  # add spaces for missing middle separator
                for asciiOffset in range(offsetThisFieldInLine + 1):
                    (thisByte,) = struct.unpack(
                        "B",
                        data[
                            offset
