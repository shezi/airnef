...
# Sony-unique codes
MTP_OP_Sony_Set_Request = (
    0x9280  # unclear exactly what these are but they appear to be wrappers for Sony-unique commands contained within
)
...
def getMtpOpDataDirection(mtpOp):
    if mtpOp in MtpOpDataDirections:
        ...
        raise AssertionError(
            "getMtpOpDataDirection called with an op 0x{:x} that is not in MtpOpDataDirections!".format(mtpOp)
        )
...
MTP_RESP_COMMUNICATION_ERROR = (
    0xFFFE  # fake response I use internally to convey error in transmitting/receiving MTP request
)
...
MtpObjAssocDescDictionary = {MTP_OBJASSOC_GenericFolder: "MTP_OBJASSOC_GenericFolder"}
...
# MTP Data Structures
#
MtpDeviceInfoTuple = namedtuple(
    "MtpDeviceInfoTuple",
    "standardVersion vendorExtensionID vendorExtensionVersion, vendorExtensionDescStr \
...
MtpStorageInfoTuple = namedtuple(
    "MtpStorageInfoTuple",
    "storageType, fileSystemType, accessCapability, maxCapacityBytes \
						 freeSpaceBytes, freeSpaceInImages, storageDescription, volumeLabel",
)
MtpObjectInfoTuple = namedtuple(
    "MtpObjectInfoTuple",
