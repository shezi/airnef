...
SSDP_DISCOVERF_ENABLE_MULTICAST_RX_ON_HOSTNAME_IF_SOCKET = (
    0x00000200  # enable multicast listening on socket created by SSDP_DISCOVERF_CREATE_EXTRA_SOCKET_FOR_HOSTNAME_IF
)
...
def discover(
    serviceNameList,
    ssdpServiceVerifyOkCallback=None,
    numAttempts=3,
    timeoutSecsPerAttempt=2,
    flags=None,
    additionalMulticastInterfacesList=None,
):
    ...
    if additionalMulticastInterfacesList:
        for ipAddressStr in additionalMulticastInterfacesList:
            try:
                ...
                raise DiscoverFailureException(
                    'Error creating additional socket for "{:s}": {:s}'.format(ipAddressStr, str(e))
                )
    ...
    for nthAttempt in range(numAttempts):
        ...
        try:
            ...
            while True:
                ...
                (socketsWithRxData, socketsReadyTxData, socketsError) = select.select(
                    sockList, [], [], float(timeoutSecsPerAttempt) - elapsedTimeThisAttemptSecs
                )
...
def createUdpSocket(discoverFlags):
    ...
    sock.setsockopt(
        socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, 31 if (discoverFlags & SSDP_DISCOVERF_USE_TTL_31) else 1
    )
    return sock
...
def setUdpSocketForMulticastReceive(sock, multicastGroupTuple, ipAddressStrOfMulticastInterface):
    ...
    sock.setsockopt(
        ...
        socket.inet_aton(multicastGroupTuple[0]) + socket.inet_aton(ipAddressStrOfMulticastInterface),
    )
...
def getHeader(ssdpMessage, headerNameStr):
    ...
    for line in messageLines:
        ...
        if line.upper().startswith(
            headerNameStr
        ):  # field names are case-insensitive, so both are compared in uppercase
