...
MTP_TCPIP_REQ_INIT_CMD_REQ = (
    0x01  # "host introduction" is arbtirary name - don't know what undocumented PTP-TCP/IP spec calls it
)
MTP_TCPIP_REQ_INIT_EVENTS = (
    0x03  # "init events" is arbtirary name - don't know what undocumented PTP-TCP/IP spec calls it
)
...
# between Host <-> Camera
#
MTP_TCPIP_PAYLOAD_ID_CmdReq = (
    0x06  # "cmd req" is arbtirary name - don't know what undocumented PTP-TCP/IP spec calls it
)
MTP_TCPIP_PAYLOAD_ID_CmdResponse = (
    0x07  # "cmd response" is arbtirary name - don't know what undocumented PTP-TCP/IP spec calls it
)
MTP_TCPIP_PAYLOAD_ID_DataStart = (
    0x09  # "data start" is arbtirary name - don't know what undocumented PTP-TCP/IP spec calls it
)
MTP_TCPIP_PAYLOAD_ID_DataPayload = (
    0x0A  # "data payload" is arbtirary name - don't know what undocumented PTP-TCP/IP spec calls it
)
MTP_TCPIP_PAYLOAD_ID_DataPayloadLast = (
    0x0C  # "data payload last" is arbtirary name - don't know what undocumented PTP-TCP/IP spec calls it
)
...
def rxPayload(s, rxProgressFunc=None):
    ...
    try:
        ...
        if len(dataPreamble) < 4:
            raise socket.error(
                errno.EBADF,
                "TCP/IP error receiving data - received insufficient payload preamble bytes (exp=4, got=0x{:x})".format(
                    len(dataPreamble)
                ),
            )
        ...
        while payloadBytesReceived < totalPayloadBytes:
            ...
            if (
                rxProgressFunc
                and payloadBytesReceived >= 8
                and (payloadId == MTP_TCPIP_PAYLOAD_ID_DataPayload or payloadId == MTP_TCPIP_PAYLOAD_ID_DataPayloadLast)
            ):
...
#
def execMtpOp_rxPayloadProgressFunc(
    totalBytesReceivedThisPayload,
    rxTxProgressFunc,
    countBytesReceivedAcrossAllPayloads,
    totalDataTransferSizeBytesExpectedAcrossAllPayloads,
):
    if rxTxProgressFunc:
        rxTxProgressFunc(
            countBytesReceivedAcrossAllPayloads + totalBytesReceivedThisPayload,
            totalDataTransferSizeBytesExpectedAcrossAllPayloads,
        )
...
def execMtpOp(s, mtpOp, cmdArgsPacked=b"", dataToSend=b"", rxTxProgressFunc=None):
    ...
    theCmdReq = (
        struct.pack(
            "<IIHI",
            MTP_TCPIP_PAYLOAD_ID_CmdReq,
            mtpDataDirToCmdReqDataDirectionCode[dataDirection],
            mtpOp,
            txTransactionId,
        )
        + cmdArgsPacked
    ...
    while True:
        ...
        try:
            ...
            data = rxPayload(
                s,
                lambda totalBytesReceivedThisPayload: execMtpOp_rxPayloadProgressFunc(
                    totalBytesReceivedThisPayload,
                    rxTxProgressFunc,
                    len(dataReceivedSoFar),
                    totalDataTransferSizeBytesExpectedAcrossAllPayloads,
                ),
            )
            (payloadId,) = struct.unpack("<I", data[0:4])
            ...
            if payloadId == MTP_TCPIP_PAYLOAD_ID_DataStart:
                ...
                if dataDirection != MTP_DATA_DIRECTION_CAMERA_TO_HOST:
                    raise MtpProtocolException(
                        "Camera Protocol Error: {:s}: Received unexpected MTP_TCPIP_PAYLOAD_ID_DataStart for non-inbound transfer".format(
                            getMtpOpDesc(mtpOp)
                        )
                    )
                ...
                if rxTransactionId != txTransactionId:
                    raise MtpProtocolException(
                        "Camera Protocol Error: {:s}: Incorrect transaction ID for MTP_TCPIP_PAYLOAD_ID_DataStart (exp={:08x}, got={:08x})".format(
                ...
                if isDebugLog():
                    applog_d(
                        "execMtpOp: {:s} - DataStart payload [expected data bytes is 0x{:x}]".format(
                            getMtpOpDesc(mtpOp), totalDataTransferSizeBytesExpectedAcrossAllPayloads
                        )
                    )
                ...
                if isDebugLog():
                    if (
                        mtpOp == MTP_OP_GetObject
                        or mtpOp == MTP_OP_GetPartialObject
                        or mtpOp == MTP_OP_GetLargeThumb
                        or mtpOp == MTP_OP_GetThumb
                    ):
                        maxBytesToDump = 64
                    else:
                        maxBytesToDump = 4096
                    applog_d(
                        "execMtpOp: {:s} - Data payload [ID {:x}] (0x{:08x} bytes):".format(
                            getMtpOpDesc(mtpOp), payloadId, len(data)
                        )
                    )
                ...
                if rxTransactionId != txTransactionId:
                    raise MtpProtocolException(
                        "Camera Protocol Error: {:s}: Incorrect transaction ID for data payload (exp={:08x}, got={:08x})".format(
                            getMtpOpDesc(mtpOp), txTransactionId, rxTransactionId
                        )
                    )
                ...
                if rxTransactionId != txTransactionId:
                    raise MtpProtocolException(
                        "Camera Protocol Error: {:s}: Incorrect transaction ID for MTP_TCPIP_PAYLOAD_ID_CmdResponse (exp={:08x}, got={:08x})".format(
                            getMtpOpDesc(mtpOp), txTransactionId, rxTransactionId
                        )
                    )
                ...
                if isDebugLog():
                    applog_d(
                        'execMtpOp: {:s} - CmdResponse payload (resp="{:s}"):'.format(
                            getMtpOpDesc(mtpOp), getMtpRespDesc(mtpRespCode)
                        )
                    )
                    applog_d(strutil.hexdump(data))

                if mtpRespCode != MTP_RESP_Ok:
                    raise MtpOpExecFailureException(
                        mtpRespCode,
                        "Camera Command Failed: {:s}, Error: {:s}".format(
                            getMtpOpDesc(mtpOp), getMtpRespDesc(mtpRespCode)
                        ),
                    )
                ...
                if (
                    totalDataTransferSizeBytesExpectedAcrossAllPayloads
                    and len(dataReceivedSoFar) < totalDataTransferSizeBytesExpectedAcrossAllPayloads
                ):
                    raise MtpProtocolException(
                        "Camera Protocol Error: {:s}: Data underrun (exp=0x{:08x}, got=0x{:08x})".format(
                            getMtpOpDesc(mtpOp),
                            totalDataTransferSizeBytesExpectedAcrossAllPayloads,
                            len(dataReceivedSoFar[8:]),
                        )
                    )
                ...
                if isDebugLog():
                    ...
                    applog_d(strutil.hexdump(data))
                raise MtpProtocolException(
                    "Camera Networking Error: {:s}: Unrecognized payload ID (0x{:08x})".format(
                        getMtpOpDesc(mtpOp), payloadId
                    )
                )
            ...
            if g_PartialRxDataPayloadData:
                ...
                if isDebugLog():
                    applog_d(
                        "execMtpOp: {:s} - Partial payload after error (0x{:08x} bytes):".format(
                            getMtpOpDesc(mtpOp), len(data)
                        )
                    )
                    applog_d(strutil.hexdump(data[: min(len(data), 1024)]))
                (rxTransactionId,) = struct.unpack("<I", data[4:8])
                if rxTransactionId != txTransactionId:
                    raise MtpProtocolException(
                        "Camera Networking Error: {:s}: Incorrect transaction ID for data payload (exp={:08x}, got={:08x})".format(
                            getMtpOpDesc(mtpOp), txTransactionId, rxTransactionId
                        )
                    )
                dataReceivedSoFar += data[8:]
            ...
            raise MtpOpExecFailureException(
                MTP_RESP_COMMUNICATION_ERROR,
                "{:s}: Socket error, partial data received - 0x{:x} of 0x{:x} bytes for specific payload, 0x{:x} of 0x{:x} of total data bytes expected. Error: {:s}".format(
                    getMtpOpDesc(mtpOp),
                    bytesReceivedLastPayload,
                    lastPayloadExpectedSize,
                    len(dataReceivedSoFar),
                    totalDataTransferSizeBytesExpectedAcrossAllPayloads,
                    str(e),
                ),
...
def sendInitCmdReq(s, guidHighLowTuple, hostNameStr, hostVerInt):
    ...
    try:
        ...
        if (
            wordResponse == 0x2 and len(rxdata) >= 8
        ):  # make sure first 32-bit word is equal to a value of 0x2 ("ACK") and has 4-byte session ID after
            return rxdata[4:]
        else:
            raise MtpProtocolException(
...
def sendInitEvents(s, sessionId):
    ...
    try:
        ...
        if wordResponse != 0x4:  # make sure first 32-bit word is equal to a value of 0x4 ("ACK")
            raise MtpProtocolException(
                "sendInitEvents(): Bad response/ACK - expected 0x04, got 0x{:x}".format(wordResponse)
            )
    except socket.error as error:
        raise
...
def sendProbeRequest(s):
    ...
    try:
        ...
        if wordResponse != 0xE:  # make sure first 32-bit word is equal to a value of 0xe ("probe response")
            raise MtpProtocolException(
                "sendProbeRequest(): Bad response/ACK - expected 0x0e, got 0x{:x}".format(wordResponse)
            )
    except socket.error as error:
        raise
...
def openConnection(
    ipAddrStr,
    verbose,
    connectionTimeoutSecs=SOCKET_TIMEOUT_CONNECT_SECS_DEFAULT,
    readWriteTimeoutSecs=SOCKET_TIMEOUT_READS_WRITES_DEFAULT,
):
    port = 15740
    ...
    try:
        ...
        if type(error) == socket.timeout:
            connectErrMsg += (
                ...
                "Wifi is enabled and that you have specified the correct IP address.".format(ipAddrStr)
            )
        else:
            if error.errno == errno.ECONNREFUSED:
                connectErrMsg += (
                    ...
                    "and that your computer is connected to its network.".format(ipAddrStr)
                )
            else:
